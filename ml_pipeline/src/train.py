...
class MalwareModelTrainer:
    ...
    def prepare_data(self, csv_path: str, test_size: float = 0.2):
        ...
        if not os.path.exists(csv_path):
            ...
            print("\n📥 Please download the Mal-API-2019 dataset:")
    ...
    def save_models(self, save_dir: str = 'saved_models/'):
        ...
        for name, model in self.models.items():
            ...
            print(f"✅ Saved {name:20s} ({size_mb:.2f} MB) -> {path}")
        
        # Save label encoder
        le_path = os.path.join(save_dir, 'label_encoder.pkl')
        joblib.dump(self.label_encoder, le_path)
        size_mb = os.path.getsize(le_path) / (1024 * 1024)
        print(f"✅ Saved {'label_encoder':20s} ({size_mb:.2f} MB) -> {le_path}")
        ...
        print(f"✅ Saved {'tfidf_vectorizer':20s} ({tfidf_size:.2f} MB) -> {tfidf_path}")
        print(f"✅ Saved {'pca_transformer':20s} ({pca_size:.2f} MB) -> {pca_path}")
        ...
        print("NEXT STEPS")
        print("="*70)
        print("\n1️⃣  Copy models to ML service:")
        print(f"   cp -r {save_dir}* ../ml-service/saved_models/")
        print("\n2️⃣  Verify models are not empty:")
